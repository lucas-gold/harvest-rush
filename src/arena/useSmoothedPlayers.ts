...
// TICK_MS in server/src/constants.ts). Rendering those positions directly
// means every player visibly jumps once per tick instead of moving
// smoothly — this exponentially smooths toward the latest known position
// every animation frame instead, which is framerate-independent and
// tolerant of network jitter (unlike fixed-interval interpolation, it
// doesn't assume updates arrive exactly TICK_MS apart).
